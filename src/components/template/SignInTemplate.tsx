		...
		<>
			<section className="w-[360px] m-auto flex flex-col gap-12 pt-[60px]">
				...
				<div className="logo">
					<div className="flex flex-col gap-4 items-center">
						...
						<div className="font-semibold ">
							<span className="text-[--color-grayscale-600] mr-2">회원이 아니신가요?</span>
				...
				<div className="flex flex-col gap-10">
					<form className="flex flex-col gap-4" onSubmit={handleSubmit(onValid, onInvalid)} id="loginForm">
						<div>
							<DefaultInput
								register={register("email", {
									required: "이메일을 입력해주세요.",
									validate: {
										email: (value) => value.includes("@") || "이메일을 입력해주세요.",
									},
								})}
								placeholder="이메일"
						...
						<div>
							...
							{errors.password && <WarningLabel text={errors.password?.message} />}
						</div>

						<div className="flex justify-between ">
							<CheckBoxButton label="로그인유지" name="signIn" value="signIn" />
							<button className="text-[--color-grayscale-500] ">비밀번호 찾기</button>
						</div>
					</form>

					<div className="flex flex-col gap-4">
						<button type="submit" form="loginForm">
							<LoginButton bgColor="bg-[--color-main-green]" textColor="text-white" text=" 로그인" border="none" />
						</button>
